...
class UserNodeMapping {
  ...
  final String?
      invoiceTrackingMacaroon; // Limited macaroon for invoice tracking (backend services)
  ...
  factory UserNodeMapping.fromFirestore(DocumentSnapshot doc) {
    ...
    return UserNodeMapping(
      ...
      lastAccessed:
          (data['last_accessed'] as Timestamp?)?.toDate() ?? DateTime.now(),
  ...
  factory UserNodeMapping.fromMap(Map<String, dynamic> data) {
    return UserNodeMapping(
      ...
      lastAccessed: data['last_accessed'] is Timestamp
          ...
          : DateTime.parse(
              data['last_accessed'] ?? DateTime.now().toIso8601String()),
      status: data['status'] ?? 'active',
      metadata: data['metadata'] as Map<String, dynamic>?,
    );
  }
  ...
  UserNodeMapping copyWith({
    ...
    return UserNodeMapping(
      ...
      invoiceTrackingMacaroon:
          invoiceTrackingMacaroon ?? this.invoiceTrackingMacaroon,
