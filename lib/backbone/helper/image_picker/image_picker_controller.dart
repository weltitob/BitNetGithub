...
class ImagePickerController extends GetxController {
  ...
  final RxInt currentView =
      0.obs; // 0 for mixed view, -1 for NFT-only, 1 for album view
  ...
  Function(AssetPathEntity? album, AssetEntity? image, MediaDatePair? pair)?
      onImageTap;
  ...
  void initialize({
    ...
    if (includeNFTs) {
      try {
        ...
        final logger = Get.find<LoggerService>();
        logger.w(
            "⚠️ ProfileController not available during registration, NFT features will be disabled");
        print(
            "⚠️ ProfileController not available during registration, NFT features will be disabled");
  ...
  void loadData() async {
    ...
    try {
      ...
      if (albums.value.isNotEmpty) {
        ...
        currentPhotos.value =
            await BitnetPhotoManager.loadImages(currentAlbum.value!, 0, 50);
        logger.i("Photo loading took: ${_loadTimer.elapsedMilliseconds}ms");
      } else {
        logger.w(
            "No albums found - permissions might be denied or no photos available");
        currentPhotos.value = [];
      }
  ...
  int compareMixedList(dynamic a, dynamic b) {
    DateTime dateA =
        a is AssetEntity ? a.modifiedDateTime : (a as MediaDatePair).date;
    DateTime dateB =
        b is AssetEntity ? b.modifiedDateTime : (b as MediaDatePair).date;
  ...
  Future<void> loadMoreImages() async {
    ...
    if (!includeNFTs || profileController == null) {
      ...
      List<AssetEntity> photos =
          await BitnetPhotoManager.loadImages(currentAlbum.value!, start, end);
      ...
      logger.i(
          "Loaded more mixed content in: ${loadMoreTimer.elapsedMilliseconds}ms");
    }
  }
  ...
  Future<void> _loadMoreMixedContent() async {
    ...
    // Load more NFTs
    List<MediaDatePair> nfts =
        await _fetchMoreNFTs(loads.value * 50, (loads.value * 50) + 50);
  ...
  Future<void> _loadMoreNFTs() async {
    List<MediaDatePair> nfts =
        await _fetchMoreNFTs(loads.value * 50, (loads.value + 1) * 50);
    currentNFTs.addAll(nfts);
    loads.value++;
  }
  ...
  Future<void> loadMetaLazy(String assetId, MediaDatePair pair) async {
    ...
    if (profileController == null) {
      print(
          "ProfileController not available, skipping meta loading for asset: $assetId");
      return;
    }
  ...
  Future<void> _applyExistingMetaData(
      String assetId, MediaDatePair pair) async {
    // Safe check for ProfileController availability
    if (profileController == null) {
      print(
          "ProfileController not available, skipping existing meta data apply");
      return;
    }
  ...
  Future<void> _fetchAndApplyMetaData(
      String assetId, MediaDatePair pair) async {
    // Safe check for ProfileController availability
    if (profileController == null) {
  ...
  void _applyMediaToPair(MediaDatePair pair, Media media) {
    ...
    if (mixedList.isNotEmpty) {
      index = mixedList.indexWhere((test) => (test is MediaDatePair &&
          test.media == null &&
    ...
    if (currentNFTs.isNotEmpty) {
      indexNft = currentNFTs.indexWhere(
          (test) => (test.media == null && test.assetId == pair.assetId));
  ...
  Future<void> loadAlbumThumbnails() async {
    ...
    albumThumbnails.value =
        await BitnetPhotoManager.loadAlbumThumbnails(albums.value);
  ...
  void removeNftFromLists(MediaDatePair pair) {
    int removeIndex = mixedList.indexWhere((test) => (test is MediaDatePair &&
        test.media == null &&
        test.assetId == pair.assetId));
    ...
    int removeIndexNft = currentNFTs.indexWhere(
        (test) => (test.media == null && test.assetId == pair.assetId));

    if (removeIndexNft != -1) {
      currentNFTs.removeAt(removeIndexNft);
      Get.find<LoggerService>()
          .i("${pair.assetId} was removed from currentNFTs at $removeIndexNft");
    } else {
      Get.find<LoggerService>()
...
class BitnetPhotoManager {
  ...
  static Future<List<AssetEntity>> loadImages(
    ...
    try {
      ...
      throw Exception(
          'Failed to load images from album: ${selectedAlbum.name}: $e');
    }
  }
  ...
  static Future<List<AssetEntity>> loadAlbumThumbnails(
    ...
    print(
        'Loaded ${thumbnails.length} album thumbnails in ${sw.elapsedMilliseconds}ms');
