...
class AnimationOptimizer {
    ...
    if (kIsWeb) {
      ...
      SchedulerBinding.instance.addPostFrameCallback((_) {
        final fps = SchedulerBinding.instance.currentFrameTimeStamp - 
                    SchedulerBinding.instance.firstFrameTimeStamp;
        
        if (fps > 100) { // If first frame took more than 100ms
  ...
  void _checkAccessibilityPreferences(BuildContext context) {
    ...
    _prefersReducedMotion = mediaQuery.disableAnimations || 
  ...
  bool get shouldDisableAllAnimations => 
...
extension AnimationPerformanceOptimizer on Animation {
  /// Returns a new animation that's optimized for performance when needed
  Animation<T> optimized<T>() {
