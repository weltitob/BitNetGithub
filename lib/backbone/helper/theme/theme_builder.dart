...
class ThemeController extends State<ThemeBuilder> {
  ...
  void _setWebDefaultValues() {
    // Changed from .w to .i
    _logger.i('[_setWebDefaultValues] Running in web mode, setting default values.');
    if (!mounted) {
      // Changed from .w to .i
      _logger.i('[_setWebDefaultValues] Widget not mounted, cannot set defaults.');
      return;
    }
    ...
    try {
      ...
      Provider.of<BitcoinPriceStream>(context, listen: false)
          .updateCurrency("USD");
      _logger.d('[_setWebDefaultValues] BitcoinPriceStream currency set to USD');

      Provider.of<CurrencyTypeProvider>(context, listen: false)
          .setCurrencyType(false);
      _logger.d('[_setWebDefaultValues] Default currency type set to false (show fiat)');
      ...
      if (!Get.isRegistered<WalletsController>()) {
        ...
        _logger.d('[_setWebDefaultValues] WalletsController already registered, setting default hideBalance to false');
      }
      ...
      setState(() {
        ...
        if (Get.isRegistered<SettingsController>()) {
          ...
          _logger.d('[_setWebDefaultValues] Updated SettingsController themeMode');
        }
        _primaryColor = Colors.white; // Use white as default for web consistency
      ...
      if (!Get.isRegistered<BitcoinController>()) {
        ...
        _logger.d('[_setWebDefaultValues] BitcoinController already registered.');
  ...
  void loadData(_) async {
    ...
    try {
      ...
      if (kIsWeb) {
        // Changed from .w to .i
        _logger.i('[loadData] Running in web mode, skipping Firebase load and using defaults.');
        return;
      }
      ...
      if (user == null) {
        // Changed from .w to .i
        _logger.i('[loadData] User is not authenticated. Cannot load or save settings.');
      ...
      _logger.d('[loadData] Authenticated user found: $uid. Checking Firestore settings...');
      ...
      if (docSnapshot.exists) {
        _logger.i('[loadData] Settings document found for user $uid. Loading data...');
        final data = docSnapshot.data();
        if (data == null) {
          _logger.e('[loadData] Settings document exists but data is null for user $uid. This should not happen.');
        ...
        _themeMode = ThemeMode.values.singleWhereOrNull((value) => value.name == rawThemeMode) ?? ThemeMode.system;
        _logger.d('[loadData] Loaded themeMode: $_themeMode');

        // Load Primary Color
        final rawColor = data['primary_color'] as int?;
        _primaryColor = rawColor == null ? AppTheme.immutableColorSchemeSeed : Color(rawColor); // Use seed color if null
        ...
        Provider.of<LocalProvider>(context, listen: false).setLocaleInDatabase(lang, locale);
        _logger.d('[loadData] Loaded locale: $lang');

        // Load Country
        final country = data['country'] as String? ?? 'US';
        Provider.of<CountryProvider>(context, listen: false).setCountryInDatabase(country);
        _logger.d('[loadData] Loaded country: $country');

        // Load Card
        final card = data['selected_card'] as String? ?? 'lightning';
        Provider.of<CardChangeProvider>(context, listen: false).setCardInDatabase(card);
        _logger.d('[loadData] Loaded selected_card: $card');

        // Load Currency
        final currency = data['selected_currency'] as String? ?? "USD";
        Provider.of<CurrencyChangeProvider>(context, listen: false).setFirstCurrencyInDatabase(currency);
        Provider.of<BitcoinPriceStream>(context, listen: false).updateCurrency(currency);
        _logger.d('[loadData] Loaded selected_currency: $currency');

        // Load Timezone
        final timezone = data['timezone'] as String? ?? "UTC";
        Provider.of<TimezoneProvider>(context, listen: false).setTimezoneInDatabase(getLocation(timezone));
        _logger.d('[loadData] Loaded timezone: $timezone');

        // Load Currency Type (Show Coin)
        final showCoin = data['showCoin'] as bool? ?? false;
        Provider.of<CurrencyTypeProvider>(context, listen: false).setCurrencyType(showCoin);
        ...
        if(mounted) { // Check if mounted before calling setState
          setState(() {
            _logger.d('[loadData] Calling setState to apply loaded themeMode and primaryColor.');
        ...
        getBTCAddresses().then((btcData) {
          _logger.i('[loadData] Successfully fetched/merged ${btcData.length} BTC addresses.');
        ...
        }, onError: (err) { // Removed stackTrace from onError
          _logger.e('[loadData] Error fetching BTC addresses: $err');
          final localAddresses = LocalStorage.instance.getStringList("btc_addresses:$uid") ?? [];
          // Changed from .w to .i
          _logger.i('[loadData] Using ${localAddresses.length} locally stored BTC addresses due to fetch error.');
          if (Get.isRegistered<WalletsController>()) {
        ...
        _logger.i('[loadData] Finished loading existing user settings successfully.');

      } else {
        // Changed from .w to .i
        _logger.i('[loadData] No settings document found for user $uid. Creating default settings...');
        await _createDefaultSettings(uid);
      }
    } catch (e) { // Removed stackTrace: s
  ...
  Future<void> _createDefaultSettings(String uid) async {
    _logger.i('[_createDefaultSettings] Creating default Firestore settings for user $uid');
    Map<String, dynamic> defaultData = {
      "theme_mode": ThemeMode.system.name,
      "lang": "en",
      "primary_color": AppTheme.immutableColorSchemeSeed!.value, // Default color seed
    ...
    try {
      ...
      _logger.i('[_createDefaultSettings] Successfully saved default settings for user $uid.');
      ...
      if(mounted) {
        Provider.of<LocalProvider>(context, listen: false).setLocaleInDatabase('en', const Locale('en'));
        Provider.of<CountryProvider>(context, listen: false).setCountryInDatabase('US');
        Provider.of<CardChangeProvider>(context, listen: false).setCardInDatabase('lightning');
        Provider.of<CurrencyChangeProvider>(context, listen: false).setFirstCurrencyInDatabase("USD");
        Provider.of<TimezoneProvider>(context, listen: false).setTimezoneInDatabase(getLocation("UTC"));
        Provider.of<BitcoinPriceStream>(context, listen: false).updateCurrency("USD");
        Provider.of<CurrencyTypeProvider>(context, listen: false).setCurrencyType(false);
      } else {
        _logger.i("[_createDefaultSettings] Widget not mounted, skipping Provider updates.");
      }


      if (!Get.isRegistered<WalletsController>()) { Get.put(WalletsController()); }
      Get.find<WalletsController>().setHideBalance(hide: false);
      if (!Get.isRegistered<BitcoinController>()) { Get.put(BitcoinController()); }
      Get.find<BitcoinController>().getChartLine("USD");
      Get.find<BitcoinController>().getpbChartline("USD");

      if (mounted) { // Check if mounted before calling setState
        setState(() {
          _logger.d('[_createDefaultSettings] Calling setState after creating default settings.');
        ...
        _logger.i("[_createDefaultSettings] Widget not mounted, cannot call setState.");
      }


    } catch (e) { // Removed stackTrace: s
      _logger.e('[_createDefaultSettings] Failed to save default settings for user $uid: $e');
  ...
  Future<List<String>> getBTCAddresses() async {
    ...
    if (kIsWeb) {
      // Changed from .w to .i
      _logger.i('[getBTCAddresses] Running in web mode, returning mock address.');
    ...
    if (user == null) {
      // Changed from .w to .i
      _logger.i('[getBTCAddresses] User not authenticated, cannot fetch or load addresses. Returning empty list.');
      return [];
    }
    final String uid = user.uid;
    ...
    try {
      ...
      final dynamic response = await listBtcAddresses(); // Use dynamic initially
      _logger.d('[getBTCAddresses] Received response from listBtcAddresses. Type: ${response.runtimeType}');

      // Ensure response is a Map before proceeding
      if (response is! Map) {
        _logger.e('[getBTCAddresses] listBtcAddresses returned unexpected type: ${response.runtimeType}. Expected Map.');
        return LocalStorage.instance.getStringList(localStorageKey) ?? []; // Fallback to local
      }
      // Now cast safely
      final Map<String, dynamic> remoteAddressesMap = Map<String, dynamic>.from(response);
      _logger.i('[getBTCAddresses] Fetched ${remoteAddressesMap.length} addresses from cloud function.');


      List<String> localAddresses = LocalStorage.instance.getStringList(localStorageKey) ?? [];
      _logger.d('[getBTCAddresses] Found ${localAddresses.length} addresses in local storage.');

      _logger.d('[getBTCAddresses] Spawning isolate to merge addresses...');
      await Isolate.spawn(
          loadAddresses, [receivePort.sendPort, remoteAddressesMap, localAddresses]);

      final mergedData = await receivePort.first as List<String>; // Wait for isolate result
      _logger.i('[getBTCAddresses] Isolate completed. Final merged address count: ${mergedData.length}');
      return mergedData;
    } catch (e) { // Removed stackTrace: s
      _logger.e('[getBTCAddresses] Error getting/merging BTC addresses: $e');
      final localAddresses = LocalStorage.instance.getStringList(localStorageKey) ?? [];
      // Changed from .w to .d (it's just debug info now)
      _logger.d('[getBTCAddresses] Returning ${localAddresses.length} locally stored addresses due to error.');
  ...
  static void loadAddresses(List<dynamic> args) async {
    ...
    print('[Isolate loadAddresses] Received ${remoteAddresses.length} remote addresses and ${localAddresses.length} local addresses.');
    ...
    print('[Isolate loadAddresses] Merged into ${mergedList.length} unique addresses.');
  ...
  Future<void> setThemeMode(ThemeMode newThemeMode) async {
    _logger.i('[setThemeMode] Attempting to set theme mode to ${newThemeMode.name}');
    try {
      final user = FirebaseAuth.instance.currentUser;
      if (!kIsWeb && user != null) {
        final String uid = user.uid;
        _logger.d('[setThemeMode] Updating theme mode in Firestore for user $uid.');
        ...
        // Changed from .w to .i
        _logger.i('[setThemeMode] Skipping Firestore update (isWeb: $kIsWeb, userAuthenticated: ${user != null})');
      }
      ...
      if (mounted) {
        setState(() {
          _logger.d('[setThemeMode] Local theme mode state updated and setState called.');
        });
      } else {
        // Changed from .w to .i
        _logger.i('[setThemeMode] Widget not mounted, cannot call setState.');
      }
      _logger.i('[setThemeMode] Theme mode successfully set locally to ${newThemeMode.name}.');
    } catch (e) { // Removed stackTrace: s
  ...
  Future<void> setPrimaryColor(Color? newPrimaryColor, bool updateDB) async {
    _logger.i('[setPrimaryColor] Attempting to set primary color to $newPrimaryColor (updateDB: $updateDB)');
    ...
    try {
      ...
      if (newPrimaryColor == Colors.white || newPrimaryColor == Colors.black) {
        _logger.d('[setPrimaryColor] Input color is white/black, resetting to default immutable seed.');
        ...
        _logger.d('[setPrimaryColor] Input color is null. Using null locally, will save white to DB if requested.');
      ...
      if (mounted) {
        setState(() {
          ...
          _logger.d('[setPrimaryColor] Local primary color state updated to $_primaryColor and setState called.');
        });
      } else {
        _primaryColor = finalColor; // Update internal state even if not mounted yet (e.g., during init)
        // Changed from .w to .i
        _logger.i('[setPrimaryColor] Widget not mounted, updated _primaryColor but did not call setState.');
      }
      ...
      if (updateDB) {
        ...
        if (!kIsWeb && user != null) {
          final String uid = user.uid;
          _logger.d('[setPrimaryColor] Updating primary color in Firestore for user $uid with value $dbValue.');
          await Databaserefs.settingsCollection
              .doc(uid)
          ...
          // Changed from .w to .i
          _logger.i('[setPrimaryColor] Skipping Firestore update (updateDB: $updateDB, isWeb: $kIsWeb, userAuthenticated: ${user != null})');
        }
      } else {
        _logger.d('[setPrimaryColor] Skipping Firestore update as updateDB is false.');
      }
      ...
      // Changed from .w to .i
      _logger.i('[setPrimaryColor] Updating local state despite potential Firestore error.');
      // Still update locally even if Firebase fails
      if (mounted) {
        setState(() {
        ...
        _primaryColor = finalColor;
        // Changed from .w to .i
        _logger.i('[setPrimaryColor] Widget not mounted during error handling, updated _primaryColor.');
      }
    }
  }
  ...
  void initState() {
    ...
    try {
      ...
      print('[ThemeController initState] CRITICAL ERROR: Could not find LoggerService via Get.find() during initialization. Logging will likely fail. Error: $e');
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      try {
        ...
        _logger.i('[ThemeController PostFrameCallback] Triggered, calling loadData.');
        loadData(_);
      } catch (e) { // Removed stackTrace: s
        print('[ThemeController PostFrameCallback] Error during setup or loadData call: $e');
        // Log using print as _logger might be faulty
        _logger.e('[ThemeController PostFrameCallback] Error during setup or loadData call: $e');
      }
    });
  }
