...
Future<String?> signLightningMessage(String message,
    {String? nodeId, String? userDid}) async {
  ...
  try {
    ...
    if (signResponse.statusCode == "200") {
      ...
      logger.i(
          "⚡ 📥 Extracted signature: '${signature.isNotEmpty ? signature.substring(0, 20) + '...' : 'EMPTY'}'");
      ...
      logger.e(
          "⚡ ❌ Lightning signing failed with status: ${signResponse.statusCode}");
    ...
    if (e is StateError) {
      logger.e(
          "⚡ ❌ 🚨 THIS IS A STATE ERROR in signLightningMessage - likely the 'Bad state: No element'!");
    }
    return null;
  }
}
...
Future<String> _loadMacaroonHex() async {
  ...
  try {
    ...
    logger.w(
        "⚠️ Using placeholder macaroon loading - implement proper authentication");
...
Future<bool> verifyLightningSignature(String message, String signature,
    {String? nodeId, String? userDid}) async {
  final logger = Get.find<LoggerService>();
  ...
  try {
    ...
    String verifyMessageUrl = LightningConfig.getLightningUrl(
        'v1/verifymessage',
    ...
    if (response.statusCode == 200) {
      ...
      logger.e(
          "❌ Lightning verification failed with status ${response.statusCode}");
      return false;
    }
  } catch (e) {
...
Future<String?> getLightningNodePubkey(
    {String? nodeId, String? userDid}) async {
  final logger = Get.find<LoggerService>();
  ...
  try {
    ...
    // Construct the Lightning REST API URL
    String getInfoUrl =
        LightningConfig.getLightningUrl('v1/getinfo', nodeId: workingNodeId);
    ...
    if (response.statusCode == 200) {
      ...
      if (pubkey.isNotEmpty) {
        logger.i(
            "✅ Lightning node pubkey retrieved: ${pubkey.substring(0, 20)}...");
