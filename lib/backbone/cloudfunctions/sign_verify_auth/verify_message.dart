...
dynamic verifyMessage(String did, String challengeId, String signature,
    {String? nodeId}) async {
  ...
  try {
    HttpsCallable callable = FirebaseFunctions.instance
        .httpsCallable('verify_lightning_signature_func');
    ...
    if (userNodeId == null) {
      ...
      logger.i(
          "🛡️ 🔍 About to call NodeMappingService.getUserNodeMapping('$did')...");

      try {
        final UserNodeMapping? nodeMapping =
            await NodeMappingService.getUserNodeMapping(did);

        logger.i(
            "🛡️ 🔍 Node mapping response: ${nodeMapping != null ? 'NOT NULL' : 'NULL'}");
        ...
        if (e is StateError) {
          logger.e(
              "🛡️ ❌ 🚨 THIS IS A STATE ERROR in getUserNodeMapping - likely the 'Bad state: No element'!");
        }
    ...
    logger.i(
        "🛡️ 📥 Response received: ${response != null ? 'NOT NULL' : 'NULL'}");
    ...
    if (response.data != null && response.data is Map) {
      ...
      responseData.forEach((key, value) {
        if (value is String && value.length > 100) {
          logger.i(
              "🛡️ 📥 Response['$key']: ${value.substring(0, 50)}... (truncated)");
        } else {
      ...
      if (responseData.containsKey('customToken')) {
        ...
        logger.i(
            "🛡️ ✅ Token preview: ${customToken.substring(0, min(customToken.length, 50))}...");
      ...
      if (responseData.containsKey('message')) {
        ...
        try {
          ...
          logger.i(
              "🛡️ 📥 Parsed message data keys: ${messageData.keys.toList()}");
      ...
      responseData.forEach((key, value) {
        logger.e(
            "🛡️ ❌   $key: ${value.runtimeType} = ${value.toString().substring(0, min(value.toString().length, 100))}");
      });
      return null;
    } else {
    ...
    if (e is StateError) {
      logger.e(
          "🛡️ ❌ 🚨 THIS IS A STATE ERROR in verifyMessage - likely the 'Bad state: No element'!");
    }
