...
Future<bool> registerIndividualLightningNode({
  ...
  try {
    ...
    if (response.data != null && response.data['success'] == true) {
      ...
      logger.i("✅ Lightning Pubkey: ${response.data['data']['lightning_pubkey']}");
      return true;
    } else {
...
Future<String> assignAvailableNode() async {
  ...
  // Available nodes
  final List<String> availableNodes = ['node4', 'node5', 'node6', 'node7', 'node8'];
  
  try {
    // TODO: Implement proper load balancing logic
    // For now, randomly select a node from available nodes
    final randomIndex = DateTime.now().millisecondsSinceEpoch % availableNodes.length;
