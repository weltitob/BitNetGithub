...
class CreateTestUser {
  ...
  static Future<bool> createOrLoginTestUser() async {
    try {
      print('🔧 DEBUG: Starting test user creation/login with BitNET auth flow...');
  ...
  static Future<bool> _doLightningRegistration() async {
    try {
      ...
      final existingMapping = await NodeMappingService.getUserNodeMapping(recoveryDid);
  ...
  static Future<bool> quickLoginTestUser() async {
    try {
      ...
      final recoveryDid = RecoveryIdentity.generateRecoveryDid(testMnemonic);
      final nodeMapping = await NodeMappingService.getUserNodeMapping(recoveryDid);
