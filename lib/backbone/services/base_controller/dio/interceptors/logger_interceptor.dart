...
class LoggerInterceptor extends Interceptor {
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) {
    ...
    if (err.response?.statusCode == 429) {
      ...
      logger.w(
          '🔄 This may indicate loop-in-loop API calls - investigate request pattern');
    }
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    ...
    if (_requestHistory[requestKey]!.length > _maxHistorySize) {
      _requestHistory[requestKey]!.removeRange(
          0, _requestHistory[requestKey]!.length - _maxHistorySize);
    }
    ...
    if (requestCount > 10) {
      logger.w(
          '⚠️  HIGH FREQUENCY DETECTED: $requestCount requests to $requestPath in last minute');
      logger.w(
          '🔁 Potential loop-in-loop API calls detected - consider rate limiting');
    } else if (requestCount > 5) {
      logger.d(
          '📈 Frequent requests: $requestCount to $requestPath in last minute');
    }
