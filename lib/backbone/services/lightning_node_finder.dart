...
class LightningNodeFinder {
  ...
  static Future<bool> _testNodeConnectivity(
      String nodeId, int timeoutSeconds) async {
    try {
      // Test basic connectivity with a simple endpoint
      String testUrl =
          LightningConfig.getLightningUrl('v1/state', nodeId: nodeId);
      ...
      _logger.d(
          "Node $nodeId response: ${response.statusCode} (responding: $isResponding)");
  ...
  static Future<List<String>> findMultipleWorkingNodes({
    ...
    for (String nodeId in allNodes) {
      ...
      if (isWorking) {
        ...
        _logger.i(
            "✅ Added working node: $nodeId (${workingNodes.length}/$maxNodes)");
      }
    }
