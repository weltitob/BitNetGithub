...
class ApiRateLimiter extends GetxService {
  ...
  Future<T> enqueue<T>({
    ...
    logger.d(
        '🚦 Queued request for endpoint: $endpoint (queue size: ${_requestQueue.length})');

    if (_requestQueue.length > 10) {
      logger.w(
          '⚠️  Large queue detected: ${_requestQueue.length} pending requests');
  ...
  void _processQueue() async {
    ...
    while (_requestQueue.isNotEmpty) {
      ...
      try {
        ...
        final delay =
            _calculateDelay(queuedRequest.apiProvider, queuedRequest.endpoint);

        if (delay > Duration.zero) {
          logger.d(
              '⏱️  Rate limit delay: ${delay.inMilliseconds}ms for ${queuedRequest.endpoint}');
  ...
  Duration _calculateDelay(String apiProvider, String endpoint) {
    ...
    if (history.length >= limit) {
      ...
      final waitTime =
          const Duration(minutes: 1) - now.difference(oldestInWindow);
      logger.w(
          '🚫 Rate limit reached for $apiProvider/$endpoint. Waiting ${waitTime.inSeconds}s');
  ...
  void _recordRequest(String apiProvider, String endpoint) {
    ...
    if (_endpointHistory[key]!.length > 100) {
      _endpointHistory[key]!
          .removeRange(0, _endpointHistory[key]!.length - 100);
    }
  }
  ...
  Map<String, dynamic> getQueueStatus() {
    return {
      ...
      'endpointHistory':
          _endpointHistory.map((key, value) => MapEntry(key, value.length)),
    };
  }
  ...
  void clearQueue() {
    logger
        .w('🧹 Clearing API request queue (${_requestQueue.length} requests)');
    while (_requestQueue.isNotEmpty) {
