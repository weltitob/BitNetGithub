...
class NodeAssignmentService {
    ...
    try {
      ...
      if (nodeUserCounts.isNotEmpty) {
        ...
        if (leastLoadedNode != null) {
          _logger.i("✅ Selected least loaded node: $leastLoadedNode (users: $minUsers)");
      ...
      int nodeIndex = DateTime.now().millisecondsSinceEpoch % availableNodes.length;
  ...
  static Future<bool> isNodeHealthy(String nodeId) async {
    try {
      ...
      if (nodeStatDoc.exists) {
        ...
        _logger.i("Node $nodeId health check: status=$status, users=$userCount/$maxUsers, healthy=$isHealthy");
        return isHealthy;
      }
  ...
  static Future<List<Map<String, dynamic>>> getNodeStatistics() async {
    try {
      ...
      stats.sort((a, b) => (a['user_count'] ?? 0).compareTo(b['user_count'] ?? 0));
