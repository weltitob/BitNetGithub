...
class AppImageCacheService extends GetxService {
  ...
  Future<Uint8List?> getFirebaseStorageImage(String storageName, {bool forceRefresh = false}) async {
    try {
      ...
      if (!forceRefresh && localStorage.hasData(cacheKey)) {
        ...
        if (DateTime.now().millisecondsSinceEpoch - timestamp < cacheExpiration.inMilliseconds) {
  ...
  Future<Uint8List?> getFaviconFromUrl(String websiteUrl, {bool forceRefresh = false}) async {
    try {
  ...
  Future<Uint8List?> getNetworkImage(String imageUrl, {bool forceRefresh = false}) async {
    try {
      final cacheKey = 'network_${Uri.parse(imageUrl).host}_${imageUrl.hashCode}';
  ...
  Future<void> preloadImages(List<String> storageNames, List<String> urls) async {
  ...
  Future<void> clearImage(String key) async {
    ...
    final keysToRemove = localStorage.getKeys()
        .whereType<String>()
  ...
  Map<String, dynamic> getCacheStats() {
    final persistentKeys = localStorage.getKeys()
        .whereType<String>()
    ...
    return {
      ...
      'memorySizeBytes': _memoryCache.values.fold(0, (sum, bytes) => sum + bytes.length),
  ...
  void _trimMemoryCache() {
    final currentSize = _memoryCache.values.fold(0, (sum, bytes) => sum + bytes.length);
