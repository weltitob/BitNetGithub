...
class _CreateAssetState extends State<CreateAsset> {
  // Helper method to build media components
  Widget _buildMediaComponent(BuildContext context, PostFile post) {
    if (post.type == MediaType.image) {
      return ImageBuilderLocal(postFile: post);
    }

    if (post.type == MediaType.attributes) {
      return AttributesBuilder(attributes: post.text!);
    }

    if (post.type == MediaType.external_url) {
      return LinkBuilder(url: post.text!);
    }

    if (post.type == MediaType.text) {
      return TextBuilderLocal(postFile: post);
    }

    if (post.type == MediaType.description) {
      return TextBuilderLocal(postFile: post);
    }

    if (post.type == MediaType.audio) {
      return AudioBuilderLocal(postfile: post);
    }

    if (post.type == MediaType.image_data) {
      return ImageBuilderLocal(postFile: post);
    }
  ...
  _pickImageFiles(MediaType mediaType) async {
    ...
    File? file =
        await ImagePickerBottomSheet(context, onImageTap: (album, image) async {
