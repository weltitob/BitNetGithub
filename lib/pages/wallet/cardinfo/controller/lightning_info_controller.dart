...
class LightningInfoController extends GetxController {
  ...
  Future<bool> getLightningPayments() async {
    ...
    try {
      ...
      List<LightningPayment> restLightningPayments =
          await listPayments(Auth().currentUser!.uid);
      LightningPaymentsList lightningPayments = LightningPaymentsList.fromJson({
        'payments': restLightningPayments,
        'first_index_offset': -1,
        'last_index_offset': -1,
        'total_num_payments': restLightningPayments.length
      });
