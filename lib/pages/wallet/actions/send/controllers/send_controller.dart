...
class SendsController extends BaseController {
  ...
  Future<void> onQRCodeScanned(dynamic encodedString, BuildContext cxt) async {
    ...
    switch (type) {
      ...
      case QRTyped.LightningMail:
        ...
        await giveValuesToLnUrl(encodedString, asAddress: true);
  ...
  Future<void> giveValuesToLnUrl(String lnUrl, {bool asAddress = false}) async {
  ...
  Future<dynamic> sendBip21BTC(BuildContext context, bool onchain,
      {bool canNavigate = true, bool shouldPop = false}) async {
      ...
      try {
        ...
        if (sendType == SendType.Bip21) {
          ...
          return await sendBip21BTC(context, bip21Mode.value == "onchain",
              canNavigate: canNavigate, shouldPop: shouldPop);
          ...
          try {
            await paymentStream.listen((dynamic response) {
              isFinished.value =
                  true; // Assuming you might want to update UI on each response
            ...
            overlayController.showOverlay("An error occurred: $e");
          }
