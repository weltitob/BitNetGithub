...
if (ps.isAuth || ps.hasAccess) {
   List<AssetPathEntity> albums = await BitnetPhotoManager.loadAlbums();
   List<AssetEntity> photos = await BitnetPhotoManager.loadImages(albums[0]);
   ImagePickerBottomSheet(context, albums: albums, recent_photos: photos, onImageTap: (album, img)async{
   ...
   BarcodeCapture? capture =  await controller.cameraController.analyzeImage(fileUrl!);
      if(capture != null) {
        ...
        showOverlay(context, "No code was found.", color: AppTheme.errorColor);
      }
   }); 
} else {
 showOverlay(context, "Please give the app photo access to use this feature.");
}
...
Future<T?> ImagePickerBottomSheet<T>(BuildContext context, {required List<AssetPathEntity> albums, required List<AssetEntity> recent_photos, required Function(AssetPathEntity album, AssetEntity image)? onImageTap}) {
  return BitNetBottomSheet<T>(
    context: context,
    width: MediaQuery.sizeOf(context).width,
    height: MediaQuery.sizeOf(context).height * 0.7,
    child: ImagePicker(albums: albums, photos: recent_photos, onImageTap: onImageTap,)
...
class ImagePicker extends StatefulWidget {
  ...
  const ImagePicker({
    super.key, required this.albums, required this.photos, this.onImageTap,
  });
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      TextButton(
        ...
        Row(
          children: [
            Text(current_album!.name, style: Theme.of(context).textTheme.labelLarge!.copyWith(
              fontSize: 18,
              decoration: TextDecoration.underline)),
            Icon(selecting_photos ? Icons.arrow_drop_down_rounded : Icons.arrow_drop_up_rounded, color: Colors.white)
          ],
        ),
        onPressed: (){
          if(selecting_photos) {
      ...
      child: (current_photos == null) ? Center(child:CircularProgressIndicator()) : 
      (selecting_photos) ?
      GridView.builder(
        itemCount: current_photos!.length,
        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 3), itemBuilder: (ctx, i) {
      ...
      itemBuilder: (ctx, i) {
        ...
        child: InkWell(
          onTap:() {current_album = widget.albums[i];
          selecting_photos = true;
          setState((){});
                  ...
                  Container(
                    ...
                    child: ClipRRect(
                      ...
                      child: AssetEntityImage(
                        ...
                        isOriginal:  false,
                        fit: BoxFit.cover,
                        thumbnailSize: ThumbnailSize.square(360),
                        
                      ),
                    ),
                  ),
                  Text(widget.albums[i].name, style: Theme.of(context).textTheme.titleSmall)
...
class BitnetPhotoManager {
  ...
  static Future<List<AssetEntity>> loadImages(AssetPathEntity selectedAlbum) async {
    List<AssetEntity> imageList = await selectedAlbum.getAssetListRange(start: 0, end: await selectedAlbum.assetCountAsync);
    return imageList;
  }
