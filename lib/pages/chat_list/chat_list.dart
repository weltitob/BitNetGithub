...
import 'package:bitnet/pages/chat_list/chat_list_view.dart';
...
import 'package:bitnet/backbone/helper/matrix_helpers/tor_stub.dart';
...
class ChatListController extends State<ChatList>
  ...
  bool get displayNavigationBar =>
      !AppTheme.isColumnMode(context) &&
      (spaces.isNotEmpty || AppTheme.separateChatTypes);
  ...
  int get selectedIndex {
    switch (activeFilter) {
      ...
      case ActiveFilter.spaces:
        return AppTheme.separateChatTypes ? 2 : 1;
    }
  }
  ...
  ActiveFilter activeFilter = AppTheme.separateChatTypes
  ...
  void editSpace(BuildContext context, String spaceId) async {
    ...
    if (mounted) {
      context.goNamed('/rooms/spaces', pathParameters: {'roomid': spaceId});
    }
  }
  ...
  void onJoinSpaceChild(SpaceRoomsChunk spaceChild) async {
    final client = Matrix.of(context).client;
    ...
    if (spaceChild.roomType == 'm.space') {
      if (spaceChild.roomId == activeSpaceId) {
        context.goNamed('/rooms/spaces', pathParameters: {'roomid': spaceChild.roomId});
      } else {
        setActiveSpace(spaceChild.roomId);
      }
      return;
    }
        context.goNamed('/rooms', pathParameters: {'roomid': spaceChild.roomId});
  }

  void onSpaceChildContextMenu([
    SpaceRoomsChunk? spaceChild,
    Room? room,
  ]) async {
    final client = Matrix.of(context).client;
    //final activeSpaceId = activeSpaceId;
    final activeSpace =
  ...
  void setActiveClient(Client client) {
    context.go('/rooms');
    setState(() {
      activeFilter = AppTheme.separateChatTypes
          ? ActiveFilter.messages
          : ActiveFilter.allChats;
      activeSpaceId = null;
  ...
  Widget build(BuildContext context) {
    ...
    //return SpaceView(this, scrollController: scrollController);
  }
}
