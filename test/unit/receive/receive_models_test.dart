...
void main() {
  group('InvoiceModel Tests', () {
    ...
    group('Copy Functionality', () {
      test(
          'should create copy with all original values when no parameters provided',
          () {
      ...
      test('should create copy with updated values', () {
        ...
        final copy = original.copyWith(
            state: 'SETTLED', payment_request: 'updated_request');
  ...
  group('BitcoinAddress Tests', () {
    group('JSON Parsing', () {
      test('should parse valid JSON correctly', () {
        ...
        expect(
            address.addr, equals('bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh'));
      });
    ...
    group('Address Validation Edge Cases', () {
      test('should handle case sensitivity correctly', () {
        ...
        final upperCaseP2PKH =
            BitcoinAddress(addr: '1A1ZP1EP5QGEFI2DMPTFTL5SLMV7DIVFNA');
        expect(upperCaseP2PKH.getAddressType(),
            equals('P2PKH')); // Still starts with 1

        final mixedCaseBech32 =
            BitcoinAddress(addr: 'BC1QXY2KGDYGJRSQTZQ2N0YRF2493P83KKFJHX0WLH');
        expect(mixedCaseBech32.getAddressType(),
            equals('Unknown')); // Bech32 should be lowercase
      });
      ...
      test('should handle very short and very long addresses', () {
        ...
        expect(shortAddress.getAddressType(),
            equals('P2PKH')); // Still starts with 1

        final shortBech32 = BitcoinAddress(addr: 'bc1q');
        expect(shortBech32.getAddressType(),
            equals('Bech32 (SegWit)')); // Still starts with bc1q

        final veryLongAddress = BitcoinAddress(addr: '1' + 'a' * 1000);
        expect(veryLongAddress.getAddressType(),
            equals('P2PKH')); // Still starts with 1
      });
    });
  ...
  group('Model Integration Tests', () {
    test('should work together in receive flow simulation', () {
      ...
      // 3. Create BIP21 URI
      final bip21Uri =
          'bitcoin:${btcAddress.addr}?lightning=${invoice.payment_request}';
    ...
    test('should handle serialization roundtrip correctly', () {
      ...
      expect(deserializedInvoice.payment_request,
          equals(originalInvoice.payment_request));
      expect(deserializedInvoice.add_index, equals(originalInvoice.add_index));
      expect(deserializedInvoice.payment_addr,
          equals(originalInvoice.payment_addr));
