...
class MockLightningReceiveScreen extends StatefulWidget {
  ...
  @override
  _MockLightningReceiveScreenState createState() =>
      _MockLightningReceiveScreenState();
}

class _MockLightningReceiveScreenState
    extends State<MockLightningReceiveScreen> {
  ...
  void _createLightningInvoice() async {
    ...
    final mockInvoice =
        'lnbc${amount}u${timestamp}pp5qqqsyqcyq5rqwzqfqqqsyqcyq5rqwzqfqqqsyqcyq5rqwzqfqypqdpl2pkx2ctnv5sxxmmwwd5kgetjypeh2ursdae8g6twvus8g6rfwvs8qun0dfjkxaq8rkx3yf5tcsyz3d73gafnh3cax9rn449d9p5uxz9ezhhypd0elx87sjle52dl8xet2zhp5760q';
...
void main() {
  ...
  group('Lightning Receive Integration Tests', () {
    testWidgets('Complete Lightning invoice creation flow',
      ...
      expect(find.text('Lightning Invoice'),
          findsNWidgets(2)); // Header and in container
    ...
    testWidgets('QR code button shows placeholder message',
        (WidgetTester tester) async {
      await tester.pumpWidget(
        MaterialApp(
          home: MockLightningReceiveScreen(),
  ...
  group('Lightning Invoice Validation', () {
    test('Validate Lightning invoice format', () {
      ...
      expect(isValidLightningInvoice('lnbc2500u1pvjluezpp5qqqsyqcyq5rqwzq'),
          isTrue);
      ...
      expect(
          isValidLightningInvoice('bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh'),
          isFalse);
...
bool isValidLightningInvoice(String invoice) {
  ...
  return validPrefixes
      .any((prefix) => invoice.toLowerCase().startsWith(prefix));
}
...
int? extractAmountFromInvoice(String invoice) {
  ...
  final matchWithMultiplier =
      RegExp(r'^(\d+)([munp])1').firstMatch(afterPrefix);
  if (matchWithMultiplier != null) {
    ...
    switch (multiplier) {
      case 'm':
        return amount *
            1000000; // milli-bitcoin (to satoshis): 1 mBTC = 1,000,000 satoshis
      case 'u':
        return amount; // micro-bitcoin (satoshis)
      case 'n':
        return amount ~/ 1000; // nano-bitcoin
      case 'p':
        return amount ~/ 1000000; // pico-bitcoin
      default:
        return amount;
    }
  }
