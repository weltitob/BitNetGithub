...
class MockOnchainReceiveScreen extends StatefulWidget {
  ...
  @override
  _MockOnchainReceiveScreenState createState() => _MockOnchainReceiveScreenState();
}
...
void main() {
  ...
  group('Onchain Receive Integration Tests', () {
    ...
    testWidgets('Warning message is displayed', (WidgetTester tester) async {
      ...
      expect(
        find.text('Send only Bitcoin to this address. Sending other cryptocurrencies will result in permanent loss.'),
        findsOneWidget,
      );
    });
    
    testWidgets('Multiple address refreshes cycle through addresses', (WidgetTester tester) async {
      await tester.pumpWidget(
        MaterialApp(
          home: MockOnchainReceiveScreen(),
        ),
  ...
  group('Bitcoin Address Validation', () {
    test('Validate Bitcoin address formats', () {
      ...
      expect(isValidBitcoinAddress('tb1qw508d6qejxtdg4y5r3zarvary0c5xw7kxpjzsx'), isTrue);
      
      // Valid P2PKH addresses (Legacy)
      expect(isValidBitcoinAddress('1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa'), isTrue);
      expect(isValidBitcoinAddress('1BvBMSEYstWetqTFn5Au4m4GFg7xJaNVN2'), isTrue);
      
      // Valid P2SH addresses (Nested SegWit)
      expect(isValidBitcoinAddress('3J98t1WpEZ73CNmQviecrnyiWrnqRhWNLy'), isTrue);
      expect(isValidBitcoinAddress('3Cbq7aT1tY8kMxWLbitaG7yT6bPbKChq64'), isTrue);
      
      // Invalid addresses
      expect(isValidBitcoinAddress(''), isFalse);
      expect(isValidBitcoinAddress('invalid'), isFalse);
      expect(isValidBitcoinAddress('lnbc10u1p3xyz...'), isFalse); // Lightning invoice
    ...
    test('Identify address types correctly', () {
      expect(getAddressType('bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh'), equals('SegWit'));
      expect(getAddressType('bc1p5d7rjq7g6rdk2yhzks9smlaqtedr4dekq08ge8ztwac72sfr9rusxg3297'), equals('Taproot'));
      expect(getAddressType('1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa'), equals('Legacy'));
      expect(getAddressType('3J98t1WpEZ73CNmQviecrnyiWrnqRhWNLy'), equals('P2SH'));
      expect(getAddressType('tb1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3q0sl5k7'), equals('SegWit'));
    });
  });
}
...
bool isValidBitcoinAddress(String address) {
  ...
  if (address.startsWith('tb1') && address.length >= 42 && address.length <= 62) {
    return _isValidBech32(address);
  }
...
bool _isValidBase58(String address) {
  ...
  final validChars = '123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz';
